...
def test_load_location_notes_content(sample_tsv_file):
    """Tests specific content from the loaded location DataFrame."""
    df = ap.load_location_notes(sample_tsv_file)
    # Check the description of the second location (index 1, column 'Description')
    assert df.iloc[1]['Description'] == 'Altar Room', "Second location description mismatch"
    # Check the danger level of the last location (index 2, column 'DangerLevel')
    assert df.iloc[2]['DangerLevel'] == 'High', "Last location danger level mismatch"
